...
dispatched by extension: \texttt{.tap}, \texttt{.tzx}, \texttt{.wav}
(tape),
\texttt{.sna}, \texttt{.z80}, \texttt{.szx} (snapshots),
\texttt{.scr} (raw screen), \texttt{.rzx} (input recording).

On Debian and Ubuntu the packaged release (built by
\texttt{tools/release\_deb.sh}) installs a system-wide
\texttt{hc91emu} with the ROMs in \texttt{/usr/share/hc91emu}; the
emulator uses that location automatically whenever a default ROM is
not found relative to the current directory:
...
\key{F2} & quick state save (one \texttt{.szx} slot, see below) \\
\key{F4} & quick state load \\
...
\key{F11} & fullscreen on/off (aspect kept, letterboxed) \\
\key{F10} / close & quit \\
\bottomrule
\end{tabularx}

The quick save slot is a single \texttt{.szx} file at
\texttt{\$XDG\_STATE\_HOME/hc91emu/quick.szx} (by default
\texttt{\textasciitilde/.local/state/hc91emu/quick.szx};
\texttt{\%APPDATA\%\textbackslash hc91emu} on Windows), so it works no
matter which directory the emulator was started from. The initial
window is $640\times480$; \opt{--scale}~\textit{N} picks another
multiplier (1--8), and the window can be resized or toggled
fullscreen freely --- the image keeps its 4:3 aspect.

\textbf{Drag and drop:} dropping a tape, snapshot, screen or RZX file
onto the window loads it. Tapes get a clean machine reset followed by
the usual autoload typing (like \opt{--autoload}); snapshots and
screens apply on the spot.
...
\subsection{Sampled recordings (\texttt{.wav})}

A \texttt{.wav} file --- a digitized recording of a genuine cassette,
e.g. captured from a real HC-91 or a preserved audio archive --- loads
through the pulse player directly. The samples get a DC-removal pass
(an exponential moving average) and a Schmitt trigger at one eighth of
the peak deviation; the resulting edge-to-edge times become tape
pulses, so a recording loads exactly like the cassette it came from.
\opt{--real-tape} is implied. Plain PCM is accepted (8- or 16-bit,
mono or stereo, 4--192\,kHz); silences longer than a second are
treated as block boundaries, so multi-load auto-pause works on sampled
tapes too. The bundled \texttt{tests/tap2wav} renders a \texttt{.tap}
into such a recording if you want to experiment.
...
\optrow{--scale N}{initial window size multiplier 1--8 (default 2)}
